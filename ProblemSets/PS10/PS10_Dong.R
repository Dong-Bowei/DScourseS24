...
# Workflow
rec_wf.tree <- workflow() %>%
  add_model(tune_tree_spec) %>%
  add_formula(high.earner ~ education + marital.status + race + workclass + occupation + relationship + sex + age + capital.gain + capital.loss + hours)

# Tuning results
rec_res.tree <- rec_wf.tree %>%
  tune_grid(
    resamples = rec_folds,
    grid = tree_parm_df
  )
...
# what is the best value of lambda?
top_acc.tree  <- show_best(rec_res.tree, metric = "accuracy")
best_acc.tree <- select_best(rec_res.tree, metric = "accuracy")
final_tree <- finalize_workflow(rec_wf.tree,
                                       best_acc.tree
)
print('*********** Tree Model **************')
...
tree_test %>% print(n = 1)
top_acc.tree %>% print(n = 1)

# combine results into a nice tibble (for later use)
tree_ans <- top_acc.tree %>% slice(1)
...
# YOU FILL IN THE REST
# Workflow
rec_wf.nnet <- workflow() %>%
  add_model(tune_nnet_spec) %>%
  add_formula(high.earner ~ education + marital.status + race + workclass + occupation + relationship + sex + age + capital.gain + capital.loss + hours)

# Tuning results
rec_res.nnet <- rec_wf.nnet %>%
  tune_grid(
    resamples = rec_folds,
    grid = nnet_parm_df
  )
...
# what is the best value of lambda?
top_acc.nnet  <- show_best(rec_res.nnet, metric = "accuracy")
best_acc.nnet <- select_best(rec_res.nnet, metric = "accuracy")
final_nnet <- finalize_workflow(rec_wf.nnet,
                                best_acc.nnet
)
print('*********** Neural Net **************')
...
nnet_test %>% print(n = 1)
top_acc.nnet %>% print(n = 1)

# combine results into a nice tibble (for later use)
nnet_ans <- top_acc.nnet %>% slice(1)
...
# what is the best value of lambda?
top_acc.knn  <- show_best(rec_res.knn, metric = "accuracy")
best_acc.knn <- select_best(rec_res.knn, metric = "accuracy")
final_knn <- finalize_workflow(rec_wf.knn,
                                best_acc.knn
)
print('*********** KNN **************')
...
knn_test %>% print(n = 1)
top_acc.knn %>% print(n = 1)

# combine results into a nice tibble (for later use)
knn_ans <- top_acc.knn %>% slice(1)
...
# what is the best value of lambda?
top_acc.svm  <- show_best(rec_res.svm, metric = "accuracy")
best_acc.svm <- select_best(rec_res.svm, metric = "accuracy")
final_svm <- finalize_workflow(rec_wf.svm,
                               best_acc.svm
)
print('*********** SVM **************')
...
svm_test %>% print(n = 1)
top_acc.svm %>% print(n = 1)

# combine results into a nice tibble (for later use)
svm_ans <- top_acc.svm %>% slice(1)
